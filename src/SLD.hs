module SLD(sld) where
    ...
    predefinedRulesMap = [("call",callSubstitution),("is",evalSubstitution),("not",notSubstitution "not"),("\\+",notSubstitution "\\+")]
    ...
    notSubstitution opCode strategy prog (Goal (Comb op goal:rest)) | op==opCode =  case strategy $ sld strategy prog (Goal goal) of
