...
{-|
    A modified version of the provided Parser Module
    capable of parsing Tuple and ';'
-}
module OurParser
  ( Parse(..), parseFile
  ) where
...
-- |Parse a goal
goal :: Parser Goal
goal = Goal <$> (whitespaces *> ((:[]) <$> semTerm) <* symbol "." <* eof)
...
rhs :: Parser [Term]
rhs = symbol "." *> pure [] <|> symbol ":-" *> ((:[])  <$> semTerm) <* symbol "."

-- |Parse semicolon separated comTerms
semTerm :: Parser Term
semTerm = do
...
-- |Parse comma separated baseTerms
comTerm :: Parser Term
comTerm = do
...
-- |Parse a list, variable, combinator or parenthesized semTerm
baseTerm :: Parser Term
baseTerm = list <|> var <|> comb <|> parens semTerm
